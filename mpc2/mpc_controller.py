...
class MPCController:
    def __init__(self):
        ...
        #self.Q = np.diag([80, 80, 80, 80, 80, 100, 50, 50, 50, 50, 50, 50])
        #self.R = np.diag([50, 80, 80, 80])
        self.Q = np.eye(12)
        self.Q[8, 8] = 5.  # z vel
        self.Q[9, 9] = 10.  # x pos
        self.Q[10, 10] = 10.  # y pos
        self.Q[11, 11] = 100.  # z pos
    ...
    def Control(self, des_state, state, tm):  # A_obs, B_obs, , x0
        ...
        for i in range(self.N):
            t += i*.01
            x_target = (state2xx(self, CircleTrajectory.getDesState(self, t)))
            print('df',CircleTrajectory.getDesState(self, t))
            if i > 0:
                cost += cp.quad_form(x_cp[:, i] - x_target, self.Q)

            cost += cp.quad_form(u_cp[:, i] , self.R)
        ...
        cost += cp.quad_form((x_cp[:, self.N] - x_target), self.Q)
